...
async function setupMockProject(page: any, projectType: 'simple' | 'complex' | 'jsonnet' | 'invalid') {
	const projects = {
		simple: {
			'collagen.json': JSON.stringify({
				attrs: { viewBox: '0 0 200 100', width: 200, height: 100 },
				children: [
					{ tag: 'rect', attrs: { x: 10, y: 10, width: 180, height: 80, fill: '#e3f2fd' } },
					{ tag: 'text', attrs: { x: 100, y: 55, 'text-anchor': 'middle' }, children: ['Simple Project'] }
				]
			})
		},
		complex: {
			'collagen.json': JSON.stringify({
				...
				children: [
					{ tag: 'rect', attrs: { x: 0, y: 0, width: 400, height: 300, fill: '#f5f5f5' } },
					{ image_path: 'logo.png', attrs: { x: 10, y: 10, width: 50, height: 50 } },
					{
						tag: 'g',
						attrs: { transform: 'translate(70, 10)' },
						children: [
							{ tag: 'text', attrs: { x: 0, y: 20, 'font-size': 18 }, children: ['Complex Project'] },
							{ tag: 'text', attrs: { x: 0, y: 40, 'font-size': 12 }, children: ['With multiple elements'] }
						]
					},
					{ svg_path: 'icon.svg', attrs: { x: 300, y: 200, width: 80, height: 80 } }
				]
			}),
			'logo.png': 'fake-png-data',
			'icon.svg': '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><circle cx="12" cy="12" r="10" fill="blue"/></svg>'
		},
		jsonnet: {
			'collagen.jsonnet': `
				local width = 300;
	...
	await page.evaluate((projectData) => {
		...
		Object.entries(projectData).forEach(([filename, content]) => {
			const file = new File([content], filename, { 
				type: filename.endsWith('.json') || filename.endsWith('.jsonnet') ? 'application/json' : 'text/plain' 
			});
...
async function simulateFileUpload(page: any, projectType: 'simple' | 'complex' | 'jsonnet' | 'invalid') {
...
test.describe('Complete User Workflows', () => {
	...
	test('should handle complex project with multiple assets', async ({ page }) => {
				...
				await page.evaluate((coords) => {
					...
					if (container) {
						const touchStart = new TouchEvent('touchstart', {
							touches: [{ clientX: coords.x, clientY: coords.y }] as any
						});
						const touchMove = new TouchEvent('touchmove', {
							touches: [{ clientX: coords.x + 50, clientY: coords.y + 30 }] as any
						});
						const touchEnd = new TouchEvent('touchend', { touches: [] as any });
...
test.describe('Performance and Edge Case Workflows', () => {
	test('should handle large project workflow', async ({ page }) => {
		...
		await page.evaluate(() => {
			...
			const file = new File([JSON.stringify(largeManifest)], 'collagen.json', { 
...
test.describe('Error Recovery Workflows', () => {
	...
	test('should handle memory constraints gracefully', async ({ page }) => {
		...
		await page.evaluate(() => {
			...
			const file = new File([JSON.stringify(hugeManifest)], 'collagen.json', { 
				type: 'application/json' 
			});
			window.mockProjectFiles = { 'collagen.json': file };
		});
