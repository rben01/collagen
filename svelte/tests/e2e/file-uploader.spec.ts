...
async function createTestFile(page: any, name: string, content: string): Promise<void> {
	await page.evaluate(
		({ name, content }) => {
			const file = new File([content], name, { type: 'application/json' });
			window.testFiles = window.testFiles || {};
			window.testFiles[name] = file;
		},
		{ name, content }
	);
}
...
test.describe('FileUploader Component', () => {
	...
	test('should display initial upload interface', async ({ page }) => {
		...
		await expect(page.locator('text=collagen.json')).toBeVisible();
		await expect(page.locator('text=collagen.jsonnet')).toBeVisible();
	...
	test('should show loading state', async ({ page }) => {
		...
		await page.waitForFunction(() => window.sjsonnet || true, { timeout: 5000 });
...
test.describe('File Upload Simulation', () => {
	test('should handle JSON file upload simulation', async ({ page }) => {
		...
		const manifest = {
			...
			children: [
				{ tag: 'rect', attrs: { x: 0, y: 0, width: 50, height: 50, fill: 'blue' } }
			]
		};

		// Simulate file upload by directly calling the component function
		await page.evaluate((manifestContent) => {
			const mockFile = new File([manifestContent], 'collagen.json', { type: 'application/json' });
...
test.describe('Drag and Drop Simulation', () => {
	...
	test('should handle file drop simulation', async ({ page }) => {
		...
		await page.evaluate(() => {
			...
			if (dropZone) {
				const mockFile = new File(['{"test": "data"}'], 'collagen.json', { type: 'application/json' });
									...
									callback([
										{
											...
											file: (cb: (file: File) => void) => {
												const file = new File(['{}'], 'collagen.json');
												cb(file);
											}
										}
									]);
...
test.describe('Visual and Responsive Design', () => {
	...
	test('should have proper hover states', async ({ page }) => {
		...
		await expect(uploadZone).toHaveCSS('background-color', /blue|#[0-9a-f]{6}/);
	});
...
test.describe('Parent Component Integration', () => {
	test('should trigger file upload callback', async ({ page }) => {
		...
		await page.evaluate(() => {
			...
			const originalCallback = window.handleFilesUploaded;
		...
		const callbackTriggered = await page.evaluate(() => window.uploadCallbackTriggered);
