...
async function generateContainerTag(
	...
	try {
		...
		const { validateDocument } = await import("../validation/index.js");

		const manifestData = await loadManifest(nestedContext.filesystem);
		const nestedRootTag = validateDocument(manifestData);
...
async function createNestedContext(
	...
	// For simplicity, we'll create a filtered view of the parent filesystem
	// that only includes files within the specified path
	const { createFileSystem } = await import("../filesystem/index.js");
	...
	for (const path of parentPaths) {
		if (path.startsWith(prefix)) {
			...
			if (nestedRelativePath) {
				...
				const fileContent = await parentContext.filesystem.load(path);
