...
export class TestFileGenerator {
	...
	static generateProjectFiles(
		...
		switch (type) {
			case "minimal":
				return {
					"collagen.json": this.createMockFile("collagen.json", "{}"),
				};
			...
			case "standard":
				...
				return {
					"collagen.json": this.createMockFile(
						"collagen.json",
			...
			case "complex":
				const complexManifest = {
					...
					children: [
						...
						...ManifestGenerator.generateRandomManifest(15, "complex")
							.children,
					],
				};
				return {
			...
			case "invalid":
				return {
					...
					"missing-ref.json": this.createMockFile(
						"missing-ref.json",
						JSON.stringify({
...
describe("Test Infrastructure", () => {
	...
	describe("ManifestGenerator", () => {
		...
		it("should generate appropriate attributes for different tags", () => {
			...
			const circleAttrs =
				ManifestGenerator.generateRandomAttributes("circle");
	...
	describe("PropertyTesting", () => {
		it("should run property-based tests", async () => {
			...
			await PropertyTesting.testProperty(
				...
				async manifest => {
					const files = {
						"collagen.json": TestFileGenerator.createMockFile(
							"collagen.json",
							JSON.stringify(manifest),
						),
					};
