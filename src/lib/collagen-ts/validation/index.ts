...
function validateFontFace(
  ...
  if (!isPlainObject(value)) {
    errors.push(
      new InvalidFieldTypeError("Font", "fonts[item]", "object", typeof value),
    );
    return null;
  }
  ...
  if (bundled) {
    ...
    if (unexpectedKeys.length > 0) {
      errors.push(new UnexpectedKeysError("BundledFontFace", unexpectedKeys));
    }
...
function validateTextTag(
  ...
  const text = obj.text;
  const isPreescaped = isBoolean(obj.is_preescaped) ? obj.is_preescaped : false;
