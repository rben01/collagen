...
Will produce the following file, link:{examples-dir}/svgs/example-01.svg[`examples-01.svg`^]:
...
Where link:{examples-dir}/example-02/collagen.json[`example-02/collagen.json`^] is below:
...
Here's link:{examples-dir}/svgs/example-02.svg[the result^] when you run `cargo run \-- -i example-02 -o example-02.svg`:
...
(No, there was no `example-03`; we skipped straight from #2 to #4.)

.link:{examples-dir}/example-04/collagen.json[`example-04/collagen.json`^]
[source,json,indent=0,tabsize=2]
----
{
...
link:{examples-dir}/example-04/collagen.json[`example-04/collagen.json`^] produces the following meme:

image::{examples-dir}/svgs/example-04.svg[width=600]
...
.link:{examples-dir}/example-05/collagen.json[`example-05/collagen.json`^]
[source,json,indent=0,tabsize=2]
----
{
	"attrs": { "viewBox": "0 0 800 650" },
...
Now, if you view the result in a desktop browser, it should look the same as above, but on a mobile device the font in the bottom pane may be use Times New Roman (or some other fallback font) instead of Impact.

image::{examples-dir}/svgs/example-05.svg[width=600]
