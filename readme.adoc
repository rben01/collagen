...
image:https://img.shields.io/npm/v/%40rben01%2Fcollagen[npm,link=https://www.npmjs.com/package/@rben01/collagen]
image:https://img.shields.io/github/license/rben01/collagen[license,link=https://github.com/rben01/collagen/blob/main/LICENSE]

toc::[]

== Quick Start

Install Collagen with `npm install -g @rben01/collagen`.
...
https://rben01.github.io/collagen/docs[Up and Running With Collagen] walks through several examples that serve as a good starting point for creating a manifest.
You can try them out immediately in the https://rben01.github.io/collagen[in-browser editor].

The in-browser editor has two modes.
Write the manifest as text, or edit the image directly: drag shapes to move them, drag their handles to resize them, draw new ones, place any image from your project, and reorder layers.
Both modes edit the same manifest file, so you can switch between them at any time.

Direct editing writes back into your manifest as a text edit, which leaves your comments, formatting, variables, and imports alone.
When several elements come from one loop, the editor says so and edits the loop body, so all of them change together.
To change one of them on its own, detach the loop, which replaces it with the elements it produced.
...
A tag's type is determined by its _primary key_: `tag` for a generic SVG element, `image_path` for an embedded image, `text` for raw text, `clgn_path` for a nested skeleton, `fonts` for embedded fonts, and `svg_path` for an included SVG file.
Every tag type, its required and optional fields, and the output it generates are documented in link:schemas/SCHEMA.md[`schemas/SCHEMA.md`].
