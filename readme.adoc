...
Meanwhile, https://en.wikipedia.org/wiki/Scalable_Vector_Graphics[veector graphics] store geometric objects such as lines, rectangles, ellipses, and even text, which have no resolution to speak of — you can zoom infinitely far on them and they'll always maintain that smooth, pixel-perfect appearance.
...
The `for` loop itself had access to the `n-spokes` variable set at the beginning of the file, which goes back to point 1: variables make things easy.
. Why SVG at all?
