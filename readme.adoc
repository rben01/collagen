...
For users: add `"$schema": "https://rben01.github.io/collagen/schemas/schema.json"` to the top-level object of your manifest to get validation and suggestions.
For developers: the tags are listed at https://docs.rs/collagen/latest/collagen/fibroblast/tags/enum.AnyChildTag.html[docs.rs/collagen].
...
+
In contrast, Collagen lets you include data (such as lists to loop over) directly in the manifest and runs via single executable with no runtime to speak of.
It also lets you write your image in a single file (`collagen.json`) instead of two (the template file and the “real code” that creates the output.)
In addition, with Collagen, there is no syntax to learn, per se; you simply write JSON.
If you reference Collagen’s https://rben01.github.io/collagen/schemas/schema.json[JSON schema], writing a Collagen manifest becomes pretty simple and convenient.
